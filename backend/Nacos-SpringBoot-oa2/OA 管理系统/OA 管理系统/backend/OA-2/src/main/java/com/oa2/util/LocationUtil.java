...
public class LocationUtil {
    ...
    // 腾讯地图API Key
    private static final String TENCENT_MAP_KEY = "XLLBZ-HYF6J-7VKF2-XZUHX-3UJN6-K6FIC";
    
    // 腾讯地图逆地理编码API URL
    private static final String TENCENT_GEOCODER_URL = "https://apis.map.qq.com/ws/geocoder/v1/";

    // 腾讯地图IP定位API URL
    private static final String TENCENT_IP_LOCATION_URL = "https://apis.map.qq.com/ws/location/v1/ip";

    /**
     * 根据IP地址获取位置信息（城市级定位）
     * @param ip 客户端IP地址，为空或本地IP时自动检测
     * @return 位置描述，例如"福建省福州市"
     */
    public static String getLocationByIp(String ip) {
        try {
            // 如果IP为空或本地回环地址，不传ip参数让腾讯自动检测
            boolean isLocalIp = (ip == null || ip.trim().isEmpty() ||
                ip.startsWith("127.") || ip.startsWith("192.168.") ||
                ip.equals("::1") || ip.equals("0:0:0:0:0:0:0:1") ||
                ip.startsWith("10.") || ip.startsWith("172."));

            StringBuilder urlBuilder = new StringBuilder(TENCENT_IP_LOCATION_URL);
            urlBuilder.append("?key=").append(TENCENT_MAP_KEY);
            if (!isLocalIp) {
                urlBuilder.append("&ip=").append(ip);
            }
            // 不传ip参数时腾讯地图会自动检测请求来源IP
            ...
            // 解析响应
            Map<String, Object> responseMap = JSONObject.parseObject(result.toString(), Map.class);
            Object statusObj = responseMap.get("status");
            Integer status = null;
            if (statusObj instanceof Integer) {
                status = (Integer) statusObj;
            } else if (statusObj instanceof String) {
                try { status = Integer.parseInt((String) statusObj); } catch (Exception e) { }
            }
            String message = (String) responseMap.get("message");

            if ((status != null && status == 0) || "query ok".equals(message)) {
                Map<String, Object> resultMap = (Map<String, Object>) responseMap.get("result");
                if (resultMap != null) {
                    Map<String, Object> adInfo = (Map<String, Object>) resultMap.get("ad_info");
                    if (adInfo != null) {
                        ...
                        if (nation != null) addr.append(nation);
                        if (province != null) addr.append(province);
                        if (city != null) addr.append(city);
                        if (district != null) addr.append(district);
